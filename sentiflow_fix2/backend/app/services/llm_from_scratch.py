...
2. Un fallback déterministe si aucun checkpoint n'est encore présent, pour que l'application fonctionne
...
def fallback_plan(question: str) -> dict[str, Any]:
    ...
    wants_collect = (not collect_negated) and any(
        word in q
        for word in [
            "recupere", "récupère", "collecte", "collecter", "nouveaux", "nouveau",
            "twitter", "tweets avec", "ajoute", "cree", "crée",
        ]
    )
    wants_compare = any(word in q for word in ["compare", "comparaison", "versus", "vs", "entre"])
    wants_timeline = any(word in q for word in ["evolution", "évolution", "tendance", "temps", "temporel", "augmente", "baisse"])
