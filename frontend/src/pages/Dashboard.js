...
function KpiCard({ label, value, sub, trend }) {
  return (
    <div className="metric-card">
      ...
      {sub && <span style={{ fontSize: '0.72rem', color: '#64748b', marginTop: 2 }}>{sub}</span>}
...
function Section({ title, hint, children }) {
  return (
    <div className="chart-card" style={{ marginBottom: 18 }}>
      <h3>{title}</h3>
      {hint && (
        <p style={{ color: '#64748b', fontSize: '0.78rem', marginTop: -4, marginBottom: 12, display: 'flex', alignItems: 'center', gap: 5 }}>
          <Info size={13} /> {hint}
        </p>
      )}
      {children}
...
function InsightsPanel({ insights }) {
  ...
  return (
    <div className="chart-card" style={{ marginBottom: 18, background: 'linear-gradient(135deg, #ffffff, #ffffff)' }}>
        ...
        {insights.map((ins, i) => {
          ...
          return (
            <div key={i} style={{
              ...
              <span style={{ fontSize: '0.86rem', color: '#1e293b', lineHeight: 1.45 }}>{ins.text}</span>
            </div>
          );
        })}
      </div>
...
export default function Dashboard() {
  ...
  if (loading) return <p style={{ color: '#94a3b8' }}>Chargement des analyses...</p>;
    ...
    <div>
      <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', flexWrap: 'wrap', gap: 10 }}>
        <div>
          ...
          <p style={{ color: '#94a3b8', fontSize: '0.85rem' }}>Veille de réputation & d'opinion — {data.targets_breakdown.length} sujet(s) suivi(s)</p>
        </div>
      ...
      <Section title="Évolution dans le temps" hint="Comment le volume de tweets positifs / négatifs évolue jour après jour. Un pic de rouge = alerte.">
        <ResponsiveContainer width="100%" height={280}>
          <AreaChart data={data.timeline}>
            ...
            <Tooltip contentStyle={{ background: '#f1f5f9', border: '1px solid #e2e8f0', borderRadius: 8 }} />
            <Legend />
            <Area type="monotone" dataKey="positive" name="Positif" stackId="1" stroke="#34d399" fill="#34d39966" />
            <Area type="monotone" dataKey="neutral" name="Neutre" stackId="1" stroke="#64748b" fill="#64748b55" />
        ...
        <Section title="Quel sujet est le mieux perçu ?" hint="Volume de tweets positifs (vert) vs négatifs (rouge) pour chaque sujet.">
          <ResponsiveContainer width="100%" height={300}>
            <BarChart data={data.targets_breakdown} layout="vertical">
              ...
              <YAxis type="category" dataKey="name" width={80} tick={{ fill: '#475569', fontSize: 11 }} />
              <Tooltip contentStyle={{ background: '#f1f5f9', border: '1px solid #e2e8f0', borderRadius: 8 }} />
              <Legend />
        ...
        <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 20 }}>
          <div>
            ...
            <div style={{ display: 'flex', flexWrap: 'wrap', gap: 6 }}>
              {data.keywords.positive.length === 0 && <span style={{ color: '#64748b', fontSize: '0.8rem' }}>—</span>}
          ...
          <div>
            ...
            <div style={{ display: 'flex', flexWrap: 'wrap', gap: 6 }}>
              {data.keywords.negative.length === 0 && <span style={{ color: '#64748b', fontSize: '0.8rem' }}>—</span>}
      ...
      <Section title="Carte des sujets" hint="Deux sujets proches sur la carte = les gens en parlent avec le même état d'esprit. Les couleurs regroupent les sujets qui se ressemblent.">
        {!advanced ? (
          ...
          <p style={{ color: '#64748b', fontSize: '0.85rem' }}>{data.pca.message}</p>
        ) : (
      ...
      <Section title="Quels sujets se ressemblent ?" hint="Plus la case est bleue, plus les deux sujets suscitent le même type de réactions. Rouge = réactions opposées.">
        {!advanced ? (
          ...
          <p style={{ color: '#64748b', fontSize: '0.85rem' }}>{data.correlation.message}</p>
        ) : (
    ...
    <div>
      <ResponsiveContainer width="100%" height={360}>
        <ScatterChart margin={{ top: 20, right: 30, bottom: 10, left: 10 }}>
          ...
          <ZAxis range={[160, 160]} />
          <Tooltip cursor={{ strokeDasharray: '3 3' }} contentStyle={{ background: '#f1f5f9', border: '1px solid #e2e8f0', borderRadius: 8 }}
            formatter={() => ['', '']} labelFormatter={() => ''} />
          <Scatter data={pca.points}>
            <LabelList dataKey="name" position="top" style={{ fill: '#1e293b', fontSize: 11 }} />
      ...
      <div style={{ display: 'flex', flexWrap: 'wrap', gap: 14, marginTop: 8 }}>
        {groups.map((g) => (
          <div key={g.id} style={{ display: 'flex', alignItems: 'center', gap: 6, fontSize: '0.8rem', color: '#475569' }}>
    ...
    <div style={{ overflowX: 'auto' }}>
      <div style={{ display: 'inline-block' }}>
        <div style={{ display: 'flex', marginLeft: 90 }}>
          {labels.map((l) => (
            <div key={l} style={{ width: cell, fontSize: '0.7rem', color: '#475569', textAlign: 'center', transform: 'rotate(-35deg)', transformOrigin: 'center', height: 40 }}>{l}</div>
          ))}
        </div>
        {matrix.map((row, i) => (
          <div key={i} style={{ display: 'flex', alignItems: 'center' }}>
            <div style={{ width: 90, fontSize: '0.72rem', color: '#475569', textAlign: 'right', paddingRight: 8 }}>{labels[i]}</div>
            {row.map((v, j) => (
              <div key={j} title={`${labels[i]} ↔ ${labels[j]} : ${simLabel(v)}`}
                style={{
                  width: cell, height: cell, background: corrColor(v),
                  display: 'flex', alignItems: 'center', justifyContent: 'center',
                  fontSize: '0.68rem', color: '#1e293b', border: '1px solid #f1f5f9', cursor: 'default',
                }}>
                {i === j ? '—' : v.toFixed(1)}
              </div>
            ))}
        ...
        <div style={{ display: 'flex', gap: 16, marginTop: 10, fontSize: '0.74rem', color: '#64748b' }}>
